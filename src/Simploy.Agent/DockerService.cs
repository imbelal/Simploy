...
public class DockerService(ILogger<DockerService> log)
{
    ...
    public async Task<string> DeployAsync(AgentDeployRequest req, CancellationToken ct)
    {
        ...
        var isSimployDockerfile = File.Exists(dockerfileFull)
            && File.ReadAllText(dockerfileFull).Contains("# Generated by Simploy", StringComparison.Ordinal);
        if ((!File.Exists(dockerfileFull) || isSimployDockerfile) && File.Exists(Path.Combine(sourceDir, "package.json")))
        {
