...
public class DockerService(IConfiguration config, ILogger<DockerService> log)
{
    ...
    public async Task<string> DeployAsync(AgentDeployRequest req, CancellationToken ct)
    {
        ...
        // If the repo does NOT track a Dockerfile, generate one from the app template
        // (or auto-detect a frontend: package.json). Every template targets its own port.
        var wantTemplate = !await IsGitTrackedAsync(sourceDir, dockerfile, ct)
            && (File.Exists(Path.Combine(sourceDir, "package.json")) || (req.Template ?? "").Equals("django", StringComparison.OrdinalIgnoreCase));
        if (wantTemplate)
        {
            log.LogInformation("Generating {Template} Dockerfile for {Repo}", req.Template ?? "static", req.GitRepository);
            hostPort = await WriteTemplateDockerfileAsync(sourceDir, req.Template ?? "static", hostPort, req.EnvVars, ct);
    ...
    /// <summary>Writes a Dockerfile for the app's template and returns the internal port it serves.</summary>
    private static async Task<int> WriteTemplateDockerfileAsync(string sourceDir, string template, int fallbackPort, IReadOnlyDictionary<string, string>? envs, CancellationToken ct)
    {
        var t = (template ?? "static").ToLowerInvariant();

        // Node-based templates get VITE_* baked as ARG/ENV before the build.
        var viteLines = new List<string>();
        foreach (var kv in envs ?? new Dictionary<string, string>())
            if (kv.Key.StartsWith("VITE_", StringComparison.OrdinalIgnoreCase))
            { viteLines.Add($"ARG {kv.Key}"); viteLines.Add($"ENV {kv.Key}=${kv.Key}"); }
        ...
        switch (t)
        {
            ...
            {
                const int port = 3000;
                var df = new List<string> { "# Generated by Simploy", "FROM node:20-alpine", "WORKDIR /app", "COPY package*.json ./", "RUN npm ci || npm install", "COPY . ." };
                df.AddRange(viteLines);
                df.AddRange(new[] { $"EXPOSE {port}", "CMD [\"npm\", \"start\"]" });
                await File.WriteAllTextAsync(Path.Combine(sourceDir, "Dockerfile"), string.Join("\n", df), ct);
                return port;
            }
            case "next":
            {
                ...
                var df = new List<string> { "# Generated by Simploy", "FROM node:20-alpine", "WORKDIR /app", "COPY package*.json ./", "RUN npm ci || npm install", "COPY . ." };
                df.AddRange(viteLines);
                df.AddRange(new[] { "RUN npm run build", $"EXPOSE {port}", "CMD [\"npm\", \"start\"]" });
                await File.WriteAllTextAsync(Path.Combine(sourceDir, "Dockerfile"), string.Join("\n", df), ct);
                return port;
            }
            case "django":
            {
                const int port = 8000;
                var df = new List<string> { "# Generated by Simploy", "FROM python:3.12-slim", "WORKDIR /app", "COPY requirements.txt ./", "RUN pip install --no-cache-dir -r requirements.txt", "COPY . .", $"EXPOSE {port}", $"CMD [\"gunicorn\", \"config.wsgi\", \"-b\", \"0.0.0.0:{port}\", \"--workers\", \"2\"]" };
                await File.WriteAllTextAsync(Path.Combine(sourceDir, "Dockerfile"), string.Join("\n", df), ct);
                return port;
            }
            default: // vite / react / static -> nginx on 8080
                await WriteStaticDockerfileAsync(sourceDir, fallbackPort, envs, ct);
                return fallbackPort;
        }
    }
